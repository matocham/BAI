...
public class SecurityConfig {
	@Configuration
	@Order(1)
	public static class PreAuthSecurityConfig {
	...
	public static class AuthSecurityConfig extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http.authorizeRequests().antMatchers("/login*", "/register*").anonymous().antMatchers("/", "/index")
					.permitAll().antMatchers("/webjars/**", "/css/**", "/js/**").permitAll().anyRequest()
